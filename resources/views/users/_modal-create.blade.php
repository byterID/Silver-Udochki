<div x-show="showCreate" x-cloak
    ...
    <div class="relative w-full max-w-lg rounded-xl bg-white p-6 shadow-xl"
        ...
        <div class="mb-5 flex items-center justify-between">
            <h2 class="text-xl font-semibold text-gray-800">Новый пользователь</h2>
            <button type="button" @click="showCreate = false" class="text-gray-400 hover:text-gray-600">✕</button>
        </div>

        @php $isCreate = old('_form') === 'create'; @endphp
        ...
        <form method="POST" action="{{ route('control-panel.users.store') }}" class="flex flex-col gap-4">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ $isCreate ? old('name') : '' }}" required
                       class="w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-indigo-400 focus:ring-indigo-400">
                @if ($isCreate) @error('name') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror @endif
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium text-gray-700">Email</label>
                <input type="email" name="email" value="{{ $isCreate ? old('email') : '' }}" required
                       class="w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-indigo-400 focus:ring-indigo-400">
                @if ($isCreate) @error('email') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror @endif
            </div>
            <div>
                ...
                <select name="role" required
                    ...
                    @forelse ($roles as $role)
                        <option value="{{ $role->value }}" @selected($isCreate && old('role') === $role->value)>{{ $role->label() }}</option>
                    @empty
                        <option value="" disabled selected>Нет ролей, которые вы можете назначить</option>
                    @endforelse
                </select>
                @if ($isCreate) @error('role') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror @endif
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium text-gray-700">Пароль</label>
                <input type="password" name="password" required autocomplete="new-password"
                       class="w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-indigo-400 focus:ring-indigo-400">
                <p class="mt-1 text-xs text-gray-400">Минимум 12 символов, буквы и цифры.</p>
                @if ($isCreate) @error('password') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror @endif
            </div>
            <div>
